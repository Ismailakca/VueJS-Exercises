        ...
        <div class="card-body">
          ...
          <div class="form-group">
            ...
            <select class="form-control" v-model="selectedProduct" @change="productSelected">
              <option selected disabled>Lütfen 1 ürün seçiniz</option>
          ...
          <div class="form-group">
            <label>Adet</label>
            <input type="text" class="form-control" placeholder="Ürün adetini giriniz.." v-model="productCount">
          </div>
          <hr>
          <button class="btn btn-primary" @click="save" :disabled="saveEnable">Kaydet</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  beforeRouteLeave(to,from,next){
    if((this.selectedProduct !== null ||  this.product.count > 0 ) && this.saveButtonClick){
